...
STATIC ADDRESS_SPACE_DESCRIPTOR mI2cDesc[LS1046A_I2C_NUM_CONTROLLERS];
  ...
  )
{
  ...
  for (Index = 0; Index < ARRAY_SIZE (mI2cDesc); Index++) {
    ...
    mI2cDesc[Index].StartDesc.AddrRangeMin = LS1046A_I2C0_PHYS_ADDRESS +
                                             (Index * LS1046A_I2C_SIZE);
    ...
    mI2cDesc[Index].StartDesc.AddrLen = LS1046A_I2C_SIZE;
  ...
  )
{
  ...
  //
  // Install the PCF2129 I2C Master protocol on this handle so the RTC driver
