...
MMIO_OPERATIONS *mMemoryOpsChassis;
  ...
  )
{
  ...
  Ver = SVR_SOC_VER (Svr);
  AsciiStrCpyS (Buf, sizeof(Buf), "SOC: ");

  for (Count = 0; Count < ARRAY_SIZE (CpuTypeList); Count++) {
    if ((CpuTypeList[Count].SocVer & SVR_WO_E) == Ver) {
      AsciiStrCatS (Buf, sizeof(Buf), (CONST CHAR8 *)CpuTypeList[Count].Name);

      if (Ver == (SVR_LX2160A & SVR_WO_E)) {
        if (!((Svr >> 12) & 0x1)) {
          AsciiStrCatS (Buf, sizeof(Buf), (CONST CHAR8 *)"C");
        }
      }

      if (IS_E_PROCESSOR (Svr)) {
        AsciiStrCatS (Buf, sizeof(Buf), (CONST CHAR8 *)"E");
      }
  ...
  if (Count == ARRAY_SIZE (CpuTypeList)) {
    AsciiStrCatS (Buf, sizeof(Buf), (CONST CHAR8 *)"unknown");
  }
  ...
  )
{
  ...
  for (Count = 0; Count < ARRAY_SIZE(Base->RcwSr); Count++) {
    UINT32 Rcw = mMemoryOpsChassis->Read32((UINTN)&Base->RcwSr[Count]);
