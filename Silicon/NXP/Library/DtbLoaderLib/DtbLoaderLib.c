...
#include <libfdt.h>
#include <Chassis2/Include/Chassis.h>
 ...
 )
{
  ...
  if (BigEndian) {
    ...
    CcbVid = mSecOps->Read32 (CryptoAddress + CCBVID_OFFSET);
  } else {
  ...
  )
{
  ...
  if (DeleteCrypto) {
    ...
    CryptoBigEndian = !!(mSecOps->Read32 (CryptoAddress + SSTA_OFFSET) & (SSTA_PLEND | SSTA_ALT_PLEND));
