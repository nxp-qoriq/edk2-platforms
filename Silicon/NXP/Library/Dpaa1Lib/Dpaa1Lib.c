...
MMIO_OPERATIONS *mMemoryOpsDpaa1Lib;
  ...
  )
{
       ...
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmRim, 0);
       /* Rx frame next engine -RISC */
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmRfne, NIA_ENG_RISC | NIA_RISC_AC_IM_RX);
       /* Rx command attribute - no order, MR[3] = 1 */
       mMemoryOpsDpaa1Lib->And32((UINTN)&Port->FmanBmRfca, ~(FMAN_BM_RFCA_ORDER | FMAN_BM_RFCA_MR_MASK));
       mMemoryOpsDpaa1Lib->Or32((UINTN)&Port->FmanBmRfca, FMAN_BM_RFCA_MR(4));
       /* enable Rx statistic counters */
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmRstc, FMAN_BM_RSTC_EN);
       /* disable Rx performance counters */
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmRpc, 0);
}
  ...
  )
{
       ...
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmTfne, NIA_ENG_RISC | NIA_RISC_AC_IM_TX);
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmTfene, NIA_ENG_RISC | NIA_RISC_AC_IM_TX);
       /* Tx command attribute - no order, MR[3] = 1 */
       mMemoryOpsDpaa1Lib->And32((UINTN)&Port->FmanBmTfca, ~(FMAN_BM_TFCA_ORDER | FMAN_BM_TFCA_MR_MASK));
       mMemoryOpsDpaa1Lib->Or32((UINTN)&Port->FmanBmTfca, FMAN_BM_TFCA_MR(4));
       /* enable Tx statistic counters */
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmTstc, FMAN_BM_TSTC_EN);
       /* disable Tx performance counters */
       mMemoryOpsDpaa1Lib->Write32((UINTN)&Port->FmanBmTpc, 0);
}
...
# define MAC_PRIVATE_ADDRESS_MASK   0x2
  ...
  DEBUG((DEBUG_INFO, "MAC addr: %02X:%02X:%02X:%02X:%02X:%02X\n",
                  MacAddrBuf->Addr[0],
                  MacAddrBuf->Addr[1],
                  MacAddrBuf->Addr[2],
                  MacAddrBuf->Addr[3],
