...
MMIO_OPERATIONS *mMemoryOpsDpaa1PhyC;
  ...
  )
{
  ...
  mMemoryOpsDpaa1PhyC->Write32((UINTN)&MdioBusRegs->MdioStat, RegValue);
  ...
  )
{
  ...
  mMemoryOpsDpaa1PhyC->Write32((UINTN)&MdioBusRegs->MdioCtl, RegValue);

  /*
   * Specify the target PHY register:
   */
  if (TRUE == Clause45)
    mMemoryOpsDpaa1PhyC->Write32((UINTN)&MdioBusRegs->MdioAddr, PhyRegNum);
   ...
   */
  mMemoryOpsDpaa1PhyC->Write32((UINTN)&MdioBusRegs->MdioData, MDIO_DATA(Value));

  /*
   * Wait until the MDIO write is complete
   */
  do {
    RegValue = mMemoryOpsDpaa1PhyC->Read32((UINTN)&MdioBusRegs->MdioData);
  ...
  )
{
   ...
   */
  if (TRUE == Clause45)
    mMemoryOpsDpaa1PhyC->Write32((UINTN)&MdioBusRegs->MdioAddr, PhyRegNum & 0xFFFF);
