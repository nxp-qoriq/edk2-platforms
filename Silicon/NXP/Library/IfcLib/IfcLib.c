  ...
  )
{
  ...
  mMemoryOpsIfc = GetMmioOperations (FeaturePcdGet (PcdIfcBigEndian));

  if (FixedPcdGetBool (PcdIfcBigEndian)) {
    Value = mMemoryOpsIfc->Read32 (Address);
  } else {
  ...
  )
{
  ...
  if (FixedPcdGetBool(PcdIfcBigEndian)) {
    return mMemoryOpsIfc->Write32 (Address, Value);
  } else {
