...
{
  MMIO_OPERATIONS *MmcOps;

  MmcOps = GetMmioOperations (FeaturePcdGet (PcdMmcBigEndian));

  return MmcOps->Read32 (Address);
}
  ...
  )
{
  ...
  return MmcOps->Write32 (Address, Value);
}
  ...
  )
{
  ...
  return MmcOps->AndThenOr32 (Address, AndData, OrData);
}
  ...
  )
{
  ...
  return MmcOps->Or32 (Address, OrData);
}
  ...
  )
{
  ...
  return MmcOps->And32 (Address, AndData);
}
