...
#define DPAA1_DEBUG_MSG(_Fmt, ...) \
  do {                                                                  \
    if (gDpaa1DebugFlags & DPAA1_DEBUG_MESSAGES_ON) {                   \
        DEBUG((DEBUG_INFO, "DPAA1 DBG: " _Fmt, ##__VA_ARGS__));      \
    }                                                                   \
  } while (0)
...
#define DPAA1_ERROR_MSG(_Fmt, ...) \
  do {                                                              \
    DEBUG((DEBUG_ERROR, "DPAA1 ERROR: " _Fmt, ##__VA_ARGS__));      \
  } while (0)
...
#define DPAA1_WARN_MSG(_Fmt, ...) \
  do {                                                              \
    DEBUG((DEBUG_INFO, "DPAA1 WARNING: " _Fmt, ##__VA_ARGS__));     \
  } while (0)
...
#define DPAA1_INFO_MSG(_Fmt, ...) \
  do {                                                              \
    DEBUG((DEBUG_INFO, "DPAA1: " _Fmt, ##__VA_ARGS__));          \
  } while (0)
