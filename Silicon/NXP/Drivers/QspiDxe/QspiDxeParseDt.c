  ...
  )
{
  ...
  BOOLEAN            Runtime;
  INT32              FdtStatus;
  MMIO_OPERATIONS  *mQspiOps;

  Status = EFI_SUCCESS;
  QspiMasterIndex = 0;
  *QSpiCount = 0;

  mQspiOps = GetMmioOperations (FeaturePcdGet (PcdQspiControllerBigEndian));
  DEBUG ((DEBUG_INFO, "ParseDeviceTree Begin\n"));
        ...
        NodeOffset = fdt_node_offset_by_compatible (Fdt, NodeOffset, (VOID *)(PcdGetPtr (PcdQspiFdtCompatible))))
  {
    ...
    DEBUG ((DEBUG_INFO, "Got status prop\n"));
    if (Prop == NULL) {
    ...
    DEBUG ((DEBUG_INFO, "Got uefi-runtime\n"));
    ...
    DEBUG ((DEBUG_INFO, "Got AmbaBase\n"));
    // determine endianness automatically
    if (MmioRead32 ( (UINTN)&QspiMasterPtr->Regs->Lutkey) == LUT_KEY) {
      DEBUG ((DEBUG_INFO, "No Swap endianness\n"));
      QspiMasterPtr->Read32 = MmioRead32;
      QspiMasterPtr->Write32 = MmioWrite32;
    } else {
      DEBUG ((DEBUG_INFO, "Swap endianness\n"));
      QspiMasterPtr->Read32 = mQspiOps->Read32;
      QspiMasterPtr->Write32 = mQspiOps->Write32;
    }

    Prop = fdt_getprop(Fdt, NodeOffset, "num-cs", &PropLen);
    DEBUG ((DEBUG_INFO, "Got num-cs\n"));
