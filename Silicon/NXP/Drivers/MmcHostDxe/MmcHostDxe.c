  ...
  if ((CmdIdx == MMC_INDX (6)) || (CmdIdx == MMC_INDX (51)) ||
    ...
    if (Instance->CardType == SD_CARD) {
      if (((CmdIdx == MMC_INDX (6)) && (LastCmd == MMC_INDX (55))) ||
          (CmdIdx == MMC_INDX (8))) {
        Status = SendCmd ((VOID *)Instance->DeviceBaseAddress,
                   &Instance->Cmd, NULL);
      }
  ...
  )
{
  ...
  RetVal = ReadBlock ((VOID *)Instance->DeviceBaseAddress, Lba, Length,
             Buffer, Instance->Cmd);
  ...
  )
{
  ...
  //send Cmd structure here, library will send this command.
  RetVal = WriteBlock ((VOID *)Instance->DeviceBaseAddress, Lba, Length,
             Buffer, Instance->Cmd);
  ...
  )
{
  ...
  NewDevicePathNode = CreateDeviceNode (MESSAGING_DEVICE_PATH, NodeSubType,
  ...
  )
{
  ...
  Status = SetIos ((VOID *)Instance->DeviceBaseAddress, BusClockFreq,
             BusWidth, TimingMode);
...
/**
  ...
  @param[in] Dtb     Dtb Image into which MMC peripheral clock
                     info is to be inserted.
  ...
  )
{
  ...
  MmcClk = gPlatformGetClockPpi.PlatformGetClock (NXP_ESDHC_CLOCK, 0);
  if (!MmcClk) {
  ...
  )
{
  ...
  MmcHostInstance = (MMC_DEVICE_INSTANCE *)AllocatePool (
                      sizeof (MMC_DEVICE_INSTANCE));
