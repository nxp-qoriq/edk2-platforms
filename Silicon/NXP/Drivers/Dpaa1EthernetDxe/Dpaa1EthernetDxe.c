  ...
  )
{
  ...
  QmanClk = NxpPlatformGetClock (NXP_I2C_CLOCK);
  if (!QmanClk) {
  ...
  if ((fdt_getprop (Dtb, NodeOffset, "big-endian", NULL) != NULL) ||
    ...
    QmanRev1 = mMemoryOpsDxe->Read32 (QmanAddress + QMAN_IP_REV_1);
    QmanRev2 = mMemoryOpsDxe->Read32 (QmanAddress + QMAN_IP_REV_2);
  } else {
  ...
  )
{
  ...
  if ((fdt_getprop (Dtb, NodeOffset, "big-endian", NULL) != NULL) ||
    ...
    BmanRev1 = mMemoryOpsDxe->Read32 (BmanAddress + BMAN_IP_REV_1);
    BmanRev2 = mMemoryOpsDxe->Read32 (BmanAddress + BMAN_IP_REV_2);
  } else {
...
#if 0 //FIX Makarand: MAC Address from EEPROM to be fixed
